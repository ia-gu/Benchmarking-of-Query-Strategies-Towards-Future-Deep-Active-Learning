    ...
    def _update_grads_val(self,grads_currX=None, first_init=False):
        ...
        if first_init:
            ...
            if self.validation_dataset is not None:
                ...
                _, self.Y_Val = next(iter(DataLoader(self.validation_dataset, shuffle = False, batch_size = len(self.validation_dataset), num_workers=8, pin_memory=True)))
                self.Y_Val = self.Y_Val.to(self.device)
            else:
                self.grads_val_curr /= predicted_y.shape[0]
                _, self.Y_new = next(iter(DataLoader(self.new_dataset, shuffle = False, batch_size = len(self.new_dataset), num_workers=8, pin_memory=True)))
