...
class Strategy:
    ...
    def update_data(self, labeled_dataset, unlabeled_dataset):
        del self.labeled_dataset, self.unlabeled_dataset
        self.labeled_dataset = labeled_dataset
        self.unlabeled_dataset = unlabeled_dataset
        del labeled_dataset, unlabeled_dataset
        ...
        with torch.no_grad():
            for batch_idx, elements_to_predict in enumerate(loop):
                ...
                if type(elements_to_predict)==list:
                        elements_to_predict = elements_to_predict[0]
                elements_to_predict = elements_to_predict.to(self.device)
                out = self.model(elements_to_predict)
    ...
    def get_grad_embedding(self, dataset, predict_labels, grad_embedding_type="bias_fc"):
        ...
        if predict_labels:
            for batch_idx, unlabeled_data_batch in enumerate(loop):
                ...
                # Calculate the fc layer gradients as well if needed
                if grad_embedding_type != "bias":
